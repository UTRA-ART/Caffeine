                                          ...
                                          int max_j)
{
  ...
  for (int j = min_j; j < max_j; j++)
  {
    for (int i = min_i; i < max_i; i++)
    {
      ...
      int free_width_lower = ((max_j - min_j) / 2) - 10;
      int free_width_upper = ((max_j - min_j) / 2) + 10;
      int free_height_upper = ((max_i - min_i) / 2) + 10;
      int free_height_upper = ((max_i - min_i) / 2) + 10;
      if (j > free_width_lower && j < free_width_upper && i > free_height_lower && j < free_height_upper){
          master_grid.setCost(i, j, FREE_SPACE);
      }
      else if (costmap_[index] == NO_INFORMATION){
        continue;
      }
      else {
